...
- **GitHub 操作（PR 作成・編集など）には `gh` CLI を使用すること。**
- **GitKraken（MCP）は使用しないこと。Git 操作はすべてターミナルで `git` / `gh` コマンドを使うこと。**
...
- **ユーザーから明示的に「修正して」「変更して」「追加して」などの指示がない限り、ソースコードを変更してはならない。**
- 確認・調査・説明のみを求められた場合は、コードを変更せずに回答すること。
- デバッグ用コード（console.log、テスト用ボタン、仮の処理など）をユーザーの許可なく追加してはならない。
- コード変更が必要だと判断した場合は、変更内容をユーザーに提案し、許可を得てから実行すること。
